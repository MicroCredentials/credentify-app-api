...
spec.beforeEach(async (ctx) => {
  ...
  const key = await new Key({}, { context }).populate({}).create(community);
...
spec.test('adds key ability', async (ctx) => {
  const context = ctx.get('context');
  ...
  ctx.is(
    ...
    && res.data.data.keyAbilities.length === 1
    && res.data.data.keyAbilities[0].kind === KeyPermissionKind.READ_ASSET, true);
