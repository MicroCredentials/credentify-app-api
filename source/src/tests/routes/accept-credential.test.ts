...
spec.test('handles unauthenticated access', async (ctx) => {
  ...
  const res = await ctx.request({
    url: `/credentials/${credential.id}/accept`,
    method: 'post',
  });
...
spec.test('marks credential as pending', async (ctx) => {
  const context = ctx.get('context');
  ...
  ctx.true((await context.mongo.db.collection('credentials').findOne({ _id: credential._id })).stage === CredentialStage.PENDING);
