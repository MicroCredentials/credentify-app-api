...
spec.test('handles unauthenticated access', async (ctx) => {
  ...
  const res = await ctx.request({
    url: `/profile/credentials/${credential.id}/cancel`,
    method: 'post',
  });
...
spec.test('marks credential as canceled', async (ctx) => {
  const context = ctx.get('context');
  ...
  ctx.true((await context.mongo.db.collection('credentials').findOne({ _id: credential._id })).stage === CredentialStage.CANCELED);
  ctx.is(res.status, 200);
});

spec.test('handles not credential owner', async (ctx) => {
  const context = ctx.get('context');
  ...
  ctx.true(res.data.errors.filter((error) => error.code === RouteErrorCode.NOT_AUTHORIZED_TO_CANCEL_CREDENTIAL).length > 0);
  ctx.is(res.status, 403);
});
