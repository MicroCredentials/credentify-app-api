...
export function inject(app: Application) {
  app.delete('/communities/:communityId/keys/:keyId', (req: Request, res: Response, next: NextFunction) => {
...
export async function resolve(req: Request, res: Response): Promise<void> {
  ...
  if (!context.hasCommunityPermission(params.communityId, CommunityPermissionKind.DELETE_KEY)) {
    throw new UnauthorizedError(RouteErrorCode.NOT_AUTHORIZED_TO_DELETE_KEY);
  }

  try {
    await key.validate();
    await key.remove(community);
  } catch (error) {
