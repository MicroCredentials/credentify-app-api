...
export function inject(app: Application) {
  app.post('/credentials/:credentialId/accept', (req: Request, res: Response, next: NextFunction) => {
    resolve(req, res).catch(next);
  });
}

/**
 * A middleware which marks the community as deleted.
...
export async function resolve(req: Request, res: Response): Promise<void> {
  ...
  credential.stage = CredentialStage.PENDING;
  try {
