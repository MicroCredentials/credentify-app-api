...
export function inject(app: Application) {
  app.get('/credentials', (req: Request, res: Response, next: NextFunction) => {
    resolve(req, res).catch(next);
  });
}

/**
 * A middleware which returns credentials.
...
export async function resolve(req: Request, res: Response): Promise<void> {
  ...
  if (!context.hasProfilePermission(ProfilePermissionKind.READ_ALL_CREDENTIALS)) {
